...
import Completecloud from '../teks/Completecloud';
...
export default function Facebook() {
  ...
  let e = Object.keys(Completecloud).sort((a, b) => a.localeCompare(b))
  ...
  return (
    ...
    <br />
      ...
      <div className={"odd"} style={mystyle}>{d.map((v, i) => (
        ...
        <Dividr id='Completecloud' name="Completecloud" />
      <div className={"even"} style={mystyle}>{e.map((v, i) => (
        <><Ch id={'k' + i} tech="Completecloud" 
        title={Completecloud[v].title}
        type={Completecloud[v].type === 'topics' ? 'topics' : 'iframe'}
        topics={Completecloud[v].topics}
         name={v} data={Completecloud[v]}
         url={Completecloud[v].url}
         /></>))}</div>
    </>
  );
}
